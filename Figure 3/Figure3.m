...
color = DefaultGreen; 
scatter(dens_Dy,magn_Dy, pointsize, color, 'filled', shape)
hold all
text(dens_Dy*.29, magn_Dy*dmag*.75, info, 'FontSize', 10,'fontweight','bold')

% Gd2O3
info = 'Gd_2O_3 NP'; %info = int2str(count); count=count+1; 
color = DefaultOrange; 
scatter(dens_Gd,magn_Gd, pointsize, color, 'filled', shape)
hold all

% Ho2O3
info = 'Ho_2O_3 NP'; %info = int2str(count); count=count+1; 
color = DefaultBlue; 
scatter(dens_Ho,magn_Ho, pointsize, color, 'filled', shape)
hold all
text(dens_Ho*.4, magn_Ho*dmag, info, 'FontSize', 10,'fontweight','bold')

% Er2O3
info = 'Er_2O_3 NP'; %info = int2str(count); count=count+1; 
color = DefaultCrimson; 
scatter(dens_Er,magn_Er, pointsize, color, 'filled', shape)
hold all
...
scatter(lin*lin*sus*energy, mag*tesla, pointsize, color, shape)
hold all
text(lin*lin*sus*energy*0.18, mag*tesla*dmag*1.4, info, 'FontSize', FontSize)
...
info = 'Dy TF (Mamin 2012)'; %info = int2str(count); count=count+1; 
color = DefaultGreen; 
